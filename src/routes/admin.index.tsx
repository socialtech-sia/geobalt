      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link to="/admin/leads" className="rounded-md bg-card border border-line text-sm text-ink px-3 py-2 flex items-center gap-1.5 hover:bg-paper">
            Заявки <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KpiCard label="Всего продуктов" value={data?.totalProducts ?? "—"} icon={Package} />
        <KpiCard label="Активных" value={data?.activeProducts ?? "—"} icon={CheckCircle2} />
        <KpiCard label="Новых заявок" value={data?.newLeads ?? "—"} icon={Inbox} />
        <KpiCard label="Заявок за 7 дней" value={data?.weekLeads ?? "—"} icon={TrendingUp} />
      </div>

      <div className="rounded-xl bg-card border border-line overflow-hidden">
        <div className="px-5 py-3 border-b border-line flex items-center justify-between">
          <h2 className="font-display font-bold text-ink">Последние заявки</h2>
          <Link to="/admin/leads" className="text-xs text-accent hover:underline">Все →</Link>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-paper-2 text-xs uppercase text-muted">
            <tr>
              <th className="text-left px-5 py-2">Дата</th>
              <th className="text-left px-5 py-2">Имя</th>
              <th className="text-left px-5 py-2">Продукт</th>
              <th className="text-left px-5 py-2">Статус</th>
            </tr>
          </thead>
          <tbody>
            {(data?.recentLeads ?? []).map((l) => (
              <tr key={l.id} className="border-t border-line hover:bg-paper-2/50">
                ...
                <td className="px-5 py-3 text-muted">{l.product_name ?? "Общий вопрос"}</td>
            ...
            {(!data?.recentLeads || data.recentLeads.length === 0) && (
              <tr><td colSpan={4} className="px-5 py-8 text-center text-muted text-sm">Пока нет заявок</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    new: { label: "Новая", cls: "bg-accent/10 text-accent border-accent/30" },
    in_progress: { label: "В работе", cls: "bg-amber-soft text-ink border-amber-soft" },
    won: { label: "Выиграна", cls: "bg-green-soft text-green border-green/30" },
